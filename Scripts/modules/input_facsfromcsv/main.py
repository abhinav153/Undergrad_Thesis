...
class OpenFaceMessage:
    ...
    def df_split(self):
        ...
        ####Bachelor Thesis###########
        self.df_au = self.df_csv.loc[:, au_regression_col] * multiplier
        ###############################
...
if __name__ == '__main__':
    ...
    ########Bachelor Thesis ###################################################
    parser.add_argument('-i','--multiplier',default=1.5,type=int,
                        help="Multiplier value for scaling intenstities of AU values")
    ###########################################################################
    ...
    ########### Bachelor Thesis ##################################
    #delete all the files in clean directory
    args = vars(args)
    if args['csv_folder'] != 'openface/default':
        filespath = Path(args['csv_folder'])
        filespath = filespath.parent / (filespath.parts[-1] + '_clean')
        files = filespath.glob('*.csv')
        for file in files:
            os.remove(file)
    ###############################################################
